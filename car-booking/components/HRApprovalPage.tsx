...
import { CheckCircle, XCircle, Clock, User, Car, Calendar, MessageSquare, Users } from 'lucide-react';
...
// Mock booking requests that have been approved by managers and are pending HR approval
const MOCK_BOOKINGS: Booking[] = [
  {
    ...
    managerComment: 'Approved for client meeting. Make sure to follow company vehicle policy.',
    numberOfDrivers: 1,
  ...
  },
  {
    ...
    managerComment: 'Team building event approved. Ensure all travelers have proper insurance.',
    numberOfDrivers: 2,
...
export function HRApprovalPage({ user, onLogout, onBookingProcessed }: HRApprovalPageProps) {
  ...
  const handleBookingAction = async (booking: Booking, action: 'approve' | 'reject') => {
    ...
    setTimeout(() => {
      ...
      const updatedBookings = bookings.map(b => 
        b.id === booking.id 
          ? { ...b, status: action === 'approve' ? 'hr_approved' : 'rejected_by_hr' }
          : b
      );
  ...
  const pendingBookings = bookings.filter(b => b.status === 'manager_approved');
  const processedBookings = bookings.filter(b => b.status !== 'manager_approved');
