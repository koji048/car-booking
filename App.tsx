...
export default function App() {
  ...
  // Handle invalid state combinations with useEffect
  useEffect(() => {
    ...
    if (currentState === 'approval' && !currentBooking && user) {
    ...
    if (user) {
      if (currentState === 'manager-approval' && user.role !== 'Manager') {
        setCurrentState('my-bookings');
        return;
      }
      if (currentState === 'hr-approval' && user.role !== 'HR') {
        setCurrentState('my-bookings');
        return;
      }
  ...
  }, [currentState, user, currentBooking]);
  ...
  const handleBookingProcessed = (booking: BookingData, action: 'approve' | 'reject', comment?: string) => {
